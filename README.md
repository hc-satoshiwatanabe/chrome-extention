...
- ダイアログ右上の「新しいタブで開く」をクリックすると、リンク先を新しいタブで開きます。元のタブ(ダイアログを開いていたタブ)を自動的に閉じるかどうかは、拡張機能アイコンのポップアップから切り替えられます(デフォルトは閉じない)。`Ctrl`/`Cmd`/`Shift`/`Alt`を押しながらクリックした場合は、この設定に関わらず通常のブラウザの新規タブ動作のままです
