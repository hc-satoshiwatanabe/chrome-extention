...
- ダイアログ右上の「新しいタブで開く」をクリックすると、リンク先を新しいタブで開き、その直後に元のタブ(ダイアログを開いていたタブ)を自動的に閉じます(タブが増え続けないようにするためです。`Ctrl`/`Cmd`/`Shift`/`Alt`を押しながらクリックした場合は、通常のブラウザの新規タブ動作のままで元のタブは閉じません)
